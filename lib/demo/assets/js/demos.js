( function( $ ) {
	...
	var fwpDemoImport = {
		...
		getDemoData: function( demo_name ) {
			...
			// Get import data
			$.ajax( {
				url: admireDemos.ajaxurl,
				type: 'get',

				data: {
					action: 'admire_ajax_get_import_data',
					demo_name: demo_name,
					security: admireDemos.admire_import_data_nonce
				},
			...
			$.ajax( {
				...
				data: {
					action : 'admire_ajax_get_demo_data',
					demo_name: demo_name,
					demo_data_nonce: admireDemos.demo_data_nonce
				},
		...
		runPopup: function( data ) {
			...
			$( '#fwp-demo-import-form' ).submit( function( e ) {
				...
				// Vars
				var demo 	= $( this ).find( '[name="admire_import_demo"]' ).val(),
					nonce 	= $( this ).find( '[name="admire_import_demo_data_nonce"]' ).val(),
					contentToImport = [];
				...
				that.importContent( {
					...
					isXML: $( '#admire_import_xml' ).is( ':checked' )
				} );
			} );
		...
		importContent: function( importData ) {
			var that = this,
				...
				timerStart = Date.now(),
				ajaxData = {
					admire_import_demo: importData.demo,
					admire_import_demo_data_nonce: importData.nonce
				};
			...
			if ( importData.contentToImport.length === 0 ) {
				...
				$.ajax( {
					...
					type: 'post',
					data: {
						action: 'admire_after_import',
						admire_import_demo: importData.demo,
						admire_import_demo_data_nonce: importData.nonce,
						admire_import_is_xml: importData.isXML
					},
					complete: function( data ) {}
				} );
				...
				complete: function( data ) {
					...
					if ( data.status === 500 || data.status === 502 || data.status === 503 ) {
						$( '.fwp-importing' )
							...
							.text( admireDemos.content_importing_error + ' '+ data.status );
			...
			importingLimit = setTimeout( function() {
				...
				$( '.fwp-importing' )
					.addClass( 'fwp-importing-failed' )
					.removeClass( 'fwp-importing' )
					.text( admireDemos.content_importing_error );
			}, 15 * 60 * 1000 );
		...
		activatePlugins: function( e ) {
			...
			$button.addClass( 'updating-message button-primary' ).html( admireDemos.button_activating );

			$.ajax( {
				url: admireDemos.ajaxurl,
				type: 'POST',
				data: {
					action : 'admire_ajax_required_plugins_activate',
					init   : $init,
				},
				...
				if ( result.success ) {
					...
					$button.removeClass( 'button-primary install-now activate-now updating-message' )
						...
						.addClass( 'disabled' )
						.text( admireDemos.button_active );
		...
		installSuccess: function( e, response ) {
			...
			$message.removeClass( 'install-now installed button-disabled updated-message' )
				...
				.html( admireDemos.button_activating );
